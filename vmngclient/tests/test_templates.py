...
from vmngclient.api.templates import (
    AttachedError,
...
class TestTemplateAPI(unittest.TestCase):
    def setUp(self):
        ...
        self.templates = [
            create_dataclass(Template, template) for template in self.data_template
        ]
    ...
    def test_wait_for_complete_false(self, mock_operation):
        ...
        # Act
        answer = test_object.wait_for_complete(
            operation_id, timeout_seconds=2, sleep_seconds=1
        )
    ...
    def test_attach_exist_template(
        self, mock_session, mock_wait_for_complete, mock_templates
    ):
    ...
    @patch.object(TemplateAPI, "templates")
    @patch.object(TemplateAPI, "wait_for_complete")
    def test_attach_no_exist_template(
        self, mock_session, mock_wait_for_complete, mock_templates
    ):
    ...
    def test_create_exception(self, mock_session, mock_templates):
        ...
        # Act
        def answer():
            return test_object.create(
                self.device_info, "template_1", "new_description", config
            )
