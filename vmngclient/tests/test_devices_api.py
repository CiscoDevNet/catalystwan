...
class TestDevicesAPI(TestCase):
    def setUp(self) -> None:
        ...
        self.devices_dataclass = [
            create_dataclass(Device, device) for device in self.devices
        ]
        self.controllers_dataclass = [
            create_dataclass(Device, device) for device in self.devices[:2]
        ]
    ...
    @patch("vmngclient.session.vManageSession")
    def test_get_reachable_devices_for_vsmatrs(
        self, mock_session, personality=Personality.VSMART
    ):
        # Arrange
        mock_session.get_data.return_value = [
            device
            for device in self.devices
            if device["personality"] == personality.value
        ]
        # Act
    ...
    @patch("vmngclient.session.vManageSession")
    def test_get_reachable_devices_for_vbonds(
        self, mock_session, personality=Personality.VBOND
    ):
        # Arrange
        mock_session.get_data.return_value = [
            device
    ...
    @patch("vmngclient.session.vManageSession")
    def test_get_reachable_devices_for_edges(
        self, mock_session, personality=Personality.EDGE
    ):
        # Arrange
        mock_session.get_data.return_value = [
            device
    ...
    @patch("vmngclient.session.vManageSession")
    def test_get_reachable_devices_for_vmanage(
        self, mock_session, personality=Personality.VMANAGE
    ):
        # Arrange
        mock_session.get_data.return_value = [
            device
    ...
    @parameterized.expand(
        [["1.1.1.1", 0], ["1.1.1.3", 1], ["1.1.1.2", 2], ["169.254.10.10", 3]]
    )
    @patch.object(DevicesAPI, "devices")
...
class TestDevicesStateAPI(TestCase):
    def setUp(self) -> None:
        ...
        self.connections_info = [
            {"system-ip": "1.1.1.1", "peer-type": "vsmart", "state": "up"},
            {"system-ip": "1.1.1.2", "peer-type": "vedge", "state": "up"},
        ]
        self.connections_info_dataclass = [
            create_dataclass(Connection, item) for item in self.connections_info
        ]
        self.reboot_history = [
            {
        ...
        self.reboot_history_dataclass = [
            create_dataclass(Reboot, item) for item in self.reboot_history
        ]
        self.device = [
            {  # vmanage
                "device-model": "vmanage",
        ...
        self.wan_interfaces_dataclass = [
            create_dataclass(WanInterface, item) for item in self.wan_interfaces
        ]
        self.bfd_session = [
            {
                "src-ip": "1.1.1.1",
        ...
        self.bfd_session_dataclass = [
            create_dataclass(BfdSessionData, item) for item in self.bfd_session
        ]
    ...
    def test_get_device_control_connections_info(self, mock_session):
        ...
        # Act
        answer = DeviceStateAPI(mock_session).get_device_control_connections_info(
            device_id="1.1.1.1"
        )
        # Assert
    ...
    def test_get_device_orchestrator_connections_info(self, mock_session):
        ...
        mock_session.get_data.return_value = self.connections_info
        # Act
        answer = DeviceStateAPI(mock_session).get_device_orchestrator_connections_info(
            device_id="1.1.1.1"
        )
        # Assert
        self.assertEqual(answer, self.connections_info_dataclass)
    ...
    def test_get_device_reboot_history(self, mock_session):
        ...
        # Act
        answer = DeviceStateAPI(mock_session).get_device_reboot_history(
            device_id="1.1.1.11"
        )
        # Assert
    ...
    def test_get_device_wan_interfaces(self, mock_session):
        ...
        # Act
        answer = DeviceStateAPI(mock_session).get_device_wan_interfaces(
            device_id="1.1.1.1"
        )
        # Assert
    ...
    def test_wait_for_bfd_session_up(self, mock_session):
        ...
        mock_session.get_data.return_value = self.bfd_session
        # Act
        answer = DeviceStateAPI(mock_session).wait_for_bfd_session_up(
            system_ip="1.1.1.1"
        )
        # Assert
