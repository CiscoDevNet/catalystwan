...
from vmngclient.api.software_action_api import (SoftwareActionAPI,
     InstallSpecification, Family, VersionType, DeviceType)
...
class TestSoftwareAcionAPI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        ['ver1', 'ver2'],'20.8', 'def_ver','mock_uuid'),}

        self.mock_devices = [{"deviceId" : "mock_uuid", "deviceIP" : 'mock_ip', 'version' : 'ver1'}]
        self.install_spec = InstallSpecification(
                            Family.VMANAGE.value, VersionType.VMANAGE.value,
                            DeviceType.VMANAGE.value)
        
        mock_session = Mock()
        self.mock_repository_object = RepositoryAPI(mock_session,[self.device_info],
        DeviceCategory.VMANAGE.value)
    ...
    def test_upgrade_software_if_downgrade_check_not_none (self, mock_get_image_version, 
        mock_downgrade_check):
        ...
        self.mock_software_action_obj.upgrade_software,'path',self.install_spec, True, True)

    @patch.object(SoftwareActionAPI,'_downgrade_check')
    @patch.object(RepositoryAPI,'get_image_version')
    def test_upgrade_software_if_downgrade_check_is_none (self, mock_get_image_version, 
        mock_downgrade_check):
    ...
    def test_wait_for_completed_success(self):
        ...
        #Assert
        answer = self.mock_software_action_obj.wait_for_completed(5,500,['Success', 'Failure'],
        'mock_action_id')
    ...
    def test_wait_for_completed_status_out_of_range(self):
        ...
        self.assertRaises(RetryError, self.mock_software_action_obj.wait_for_completed,1,1,['Success', 'Failure'],
        'mock_action_id')
