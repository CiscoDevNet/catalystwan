...
from vmngclient.api.task_status_api import TaskStatus, wait_for_completed, get_all_tasks
...
class TestTaskStatusApi(unittest.TestCase):
    ...
    @patch("vmngclient.session.vManageSession")
    def test_raise_index_error_actionid_in_tasks_ids_data_exists(self,
        mock_session,mock_get_tasks,mock_sleep):
        #Arrange
    ...
    @patch("vmngclient.api.task_status_api.get_all_tasks")
    @patch("vmngclient.session.vManageSession")
    def test_raise_index_error_actionid_in_tasks_ids_data_dosnt_exists(self,
        mock_session,mock_get_tasks,mock_sleep):
        #Arrange
    ...
    @patch("vmngclient.api.task_status_api.get_all_tasks")
    @patch("vmngclient.session.vManageSession")
    def test_raise_index_error_actionid_not_in_tasks(self,
         mock_session,mock_get_tasks,mock_sleep):
        #Arrange
        mock_session.get_data.side_effect = IndexError()
    ...
    def test_get_all_tasks(self,mock_session):
        #Arrange
        mock_session.get_json.return_value = {"runningTasks": [{"processId":"processId_1"},{"processId":"processId_2"}]}
        #Act
