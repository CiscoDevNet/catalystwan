...
class RebootAction(DeviceActionAPI):
    ...
    def execute(self):
        ...
        response = self.session.post(
            "/dataservice/device/action/reboot", json=body
        ).json()
        if response.get("id"):
    ...
    def wait_for_completed(
        ...
        def wait_for_come_up():
            ...
            try:
                ...
                logger.debug(
                    f"Status of device {self.dev.hostname} reboot is: {action_data}"
                )
            except IndexError:
                action_data = ""
...
class ValidateAction(DeviceActionAPI):  # TODO check
    ...
    def execute(self, valid: bool = True):
        ...
        response = self.session.post(
            url="/dataservice/certificate/save/vedge/list", json=body
        ).json()
        if response.get("id"):
            self.action_id = response["id"]
        else:
        ...
        def wait_for_come_up():
            ...
            if status.reachability.value == expected_reachability:
                return self.session.get_data(
                    f"/dataservice/device?host-name={self.dev.hostname}"
                )[0]
            else:
                return None
