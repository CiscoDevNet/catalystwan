...
class TlsProfile(ConvertBoolToStringModel):
    ...
    ciphersuite_list: Optional[List] = Field(json_schema_extra={'data_path': ['ciphersuite'], 'vmanage_key': 'ciphersuite-list'})
...
class Server(ConvertBoolToStringModel):
    ...
    custom_profile: Optional[bool] = Field(False, json_schema_extra={'data_path': ['tls', 'tls-properties'], 'vmanage_key': 'custom-profile'})
    profile: Optional[str] = Field(json_schema_extra={'data_path': ['tls', 'tls-properties']})
    model_config = ConfigDict(populate_by_name=True)
...
class CiscoLoggingModel(FeatureTemplate, ConvertBoolToStringModel):
    ...
    enable: Optional[bool] = Field(json_schema_extra={'data_path': ['disk']})
    size: Optional[int] = Field(json_schema_extra={'data_path': ['disk', 'file']})
    rotate: Optional[int] = Field(json_schema_extra={'data_path': ['disk', 'file']})
    tls_profile: Optional[List[TlsProfile]] = Field(json_schema_extra={'vmanage_key': 'tls-profile'})
    server: Optional[List[Server]]
    ipv6_server: Optional[List[Ipv6Server]] = Field(json_schema_extra={'vmanage_key': 'ipv6-server'})
